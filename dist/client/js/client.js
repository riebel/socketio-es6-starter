	...
	var Client = function () {
	    ...
	    _createClass(Client, [{
	        ...
	        value: function startChat(nick) {
	            ...
	            this.socket = (0, _socket2.default)({ query: "name=" + this.userName });
	            this.setupSocket();
	
	            this.chat = new _client2.default(this.socket, this.userName);
	            this.setupChat();
	        ...
	        value: function setupSocket() {
	            ...
	            this.socket.on('dong', function () {
	                _this2.latency = Date.now() - _this2.startPingTime;
	                _this2.chat.addSystemLine('Ping: ' + _this2.latency + 'ms');
	            });
	            ...
	            this.socket.on('userDisconnect', function (data) {
	                _this2.chat.addSystemLine('<b>' + (data.name.length < 1 ? 'Anon' : data.name) + '</b> disconnected.');
	            });
	
	            this.socket.on('userJoin', function (data) {
	                _this2.chat.addSystemLine('<b>' + (data.name.length < 1 ? 'Anon' : data.name) + '</b> joined.');
	            });
	
	            this.socket.on('serverSendUserChat', function (data) {
	                _this2.chat.addChatLine(data.sender, data.message, false);
	            });
	        }
	    }, {
	        key: 'setupChat',
	        value: function setupChat() {
	            ...
	            this.chat.registerCommand('ping', 'Check your latency.', function () {
	                _this3.checkLatency();
	            });
	
	            this.chat.registerCommand('help', 'Information about the chat commands.', function () {
	                _this3.chat.printHelp();
	            });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ChatClient = function () {
	    function ChatClient(socket, nick) {
	        var _this = this;
	        ...
	        input.addEventListener('keypress', function (key) {
	            ...
	            if (key === 13) {
	                _this.sendChat((0, _util.sanitizeString)(input.value));
	                input.value = '';
	            }
	        });
